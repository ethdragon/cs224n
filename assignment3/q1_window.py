...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        padded = [start * window_size] + sentence + [end * window_size]
        for i in range(len(sentence)):
            row_array = padded[i : i + (2 * window_size + 1)]
            flattened = [x for row in row_array for x in row]
            windowed_data.append((flattened, labels[i]))
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        n_window_features = Config.n_window_features
