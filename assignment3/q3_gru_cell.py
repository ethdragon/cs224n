...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('W_z', shape=[self.input_size, self.state_size], initializer=initializer)
            U_z = tf.get_variable('U_z', shape=[self.state_size, self.state_size], initializer=initializer)
            b_z = tf.get_variable('b_z', shape=[self.state_size], initializer=initializer)

            W_r = tf.get_variable('W_r', shape=[self.input_size, self.state_size], initializer=initializer)
            U_r = tf.get_variable('U_r', shape=[self.state_size, self.state_size], initializer=initializer)
            b_r = tf.get_variable('b_r', shape=[self.state_size], initializer=initializer)

            W_o = tf.get_variable('W_o', shape=[self.input_size, self.state_size], initializer=initializer)
            U_o = tf.get_variable('U_o', shape=[self.state_size, self.state_size], initializer=initializer)
            b_o = tf.get_variable('b_o', shape=[self.state_size], initializer=initializer)
